...
const MessageCard = ({message}) => {
    ...
    const {setUreadCount} = useGlobalContext();

    const handleReadClick = async ()=>{
        const read = await markMessageAsRead({messageId: message._id});
        setUreadCount((prevCount)=>(read ? prevCount -1 : prevCount +1));
        setIsRead(read)
        toast.success(`Marked As ${read ? 'Read' : 'New'}`)
    }
    const handleDeleteClick = async ()=>{
        await deleteMessage(message._id );
        setUreadCount((prevCount) => (isRead ? prevCount - 1 : prevCount + 1));
    ...
    return (
      <div className="relative bg-white p-4 rounded-md shadow-md border-gray-200">
        ...
        <h2 className="text-xl mb-4">
          {message.name}
          <span className="font-bold ml-5">Property Inquiry:</span>{" "}
